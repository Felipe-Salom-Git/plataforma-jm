...
export function GeneratePdfButton({ data, presupuestoId, userId, onSuccess }: GeneratePdfButtonProps) {
    ...
    const handleGenerate = async () => {
        try {
            ...
            // Use the exported storage instance from config to ensure same app context
            const storageRef = ref(storage, `tenants/${userId}/quotes/${presupuestoId}.pdf`);

            await uploadBytes(storageRef, blob);
            const downloadUrl = await getDownloadURL(storageRef);

            // 4. Update Firestore using generic path construction
            await updateDoc(doc(db, 'tenants', userId, 'quotes', presupuestoId), {
                pdfUrl: downloadUrl,
